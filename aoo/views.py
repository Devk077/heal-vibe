...
def generate_meal_plan_table(meal_plan, table_style):
    ...
    headers = ["Name", "Calories", "Carbs", "Protein", "Fat"]
    ...
    for timing in ['after_bed', 'breakfast', 'lunch', 'snacks', 'dinner', 'before_bed', 'pre_workout', 'post_workout']:
        ...
        if meal_items:
            ...
            for i in range(-5, 0):
                table_style.add('BACKGROUND', (i, len(meal_items_data) - 1), (i, len(meal_items_data) - 1), colors.Color(196/255, 232/255, 99/255))

            for meal_item in meal_items:
                # Set background color of food cells alternately
                background_color = colors.white if idx % 2 == 0 else colors.Color(red=220/255, green=220/255, blue=220/255)
                idx += 1
                
                # Add a row for the meal item
                meal_items_data.append([
                    Paragraph(meal_item.name, food_styles),
                    Paragraph(str(meal_item.calories), normal_style),
                    Paragraph(str(meal_item.carbs), food_styles_centered),
                    Paragraph(str(meal_item.protein), normal_style),
                    Paragraph(str(meal_item.fat), normal_style)
                ])
                
                # Set background color for food cells
                for i in range(-5, 0):
                    table_style.add('BACKGROUND', (i, len(meal_items_data) - 1), (i, len(meal_items_data) - 1), background_color)

    # Create the table
    meal_items_table = Table(meal_items_data, repeatRows=1)
    meal_items_table.setStyle(table_style)

    green_color = colors.Color(231/255,237/255,213/255)
    table_style_border = TableStyle([

        # Outside border
        ('LINEABOVE', (0, 0), (-1, 0), 1, green_color),  # Line grabove the first row
        ('LINEBELOW', (0, -1), (-1, -1), 1, green_color),  # Line below the last row
        ('LINEBEFORE', (0, 0), (0, -1), 1, green_color),  # Line before the first column
        ('LINEAFTER', (-1, 0), (-1, -1), 1, green_color),  # Line after the last column    
    ])
...
def create_heading_cell(doctor_name, designation, mobile_number):
    ...
    ], style=[('BACKGROUND', (0, 0), (-1, -1), colors.Color(206/255, 233/255, 134/255))], colWidths=[350, '*', 100])  # Adjusted column width
...
def generate_bmi_classification_table():
    ...
    green_color = colors.Color(231/255,237/255,213/255)
    table_style = [
        ('BACKGROUND', (0, 0), (-1, 0), white_color),  # Background color for header row
        ...
        ('LINEBELOW', (0, -1), (-1, -1), 1, green_color),  # Line below the last row
        ('LINEBEFORE', (0, 0), (0, -1), 1, green_color),  # Line before the first column
        ('LINEAFTER', (-1, 0), (-1, -1), 1, green_color),
    ]
...
def generate_health_clinic_table():
    ...
    text = "* Manoeuvre Weight Gain\n" \
           "* Manage Fat Loss\n" \
           "* Provide Guidance In Building Muscle / Body Building\n" \
           "* Provide Sports Nutrition\n" \
           "* Manage Diabetes / Blood Sugar\n" \
           "* Manage Cholesterol\n" \
           "* Manage Blood Pressure (BP)\n" \
           "* Manage Menopause\n" \
           "* Manage Poly Cystic Ovary Syndrome (PCOS) / Hormonal Imbalance\n" \
           "* Improve Skin Health\n" \
           "* Provide Diets For Prenatal, Antenatal & Postnatal Care"

    # Define the style for the table
    light_green = colors.Color(231/255, 237/255, 213/255)
    dark_green = colors.Color(206/255, 233/255, 134/255)
    table_style = [
        ('BACKGROUND', (0, 0), (-1, -1), light_green),  # Background color for the cell
        ('TEXTCOLOR', (0, 0), (-1, -1), colors.black),  # Text color
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),  # Alignment
        ('BOX', (0, 0), (-1, -1), 1, dark_green),  # Outer border
    ]
...
def generate_body_composition_table(appointment_id):
    ...
    if body_composition:
        data = [
            ...
            ["FAT %", f"{round(body_composition.body_fat, 2)}"],
            ["M MASS %", f"{round(body_composition.visceral_fat, 2)}"],
            ["B-AGE", f"{round(body_composition.age, 2)}"],
            ["BMR", f"{round(body_composition.RMR, 2)}"],
