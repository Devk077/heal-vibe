...
class Food(models.Model):
    ...
    fat = models.PositiveIntegerField(verbose_name='Fat(Kcal/g )')
...
class Patient(models.Model):
    ...
    name = models.CharField(max_length=100)
    phone = models.CharField(max_length=20, default='', null=True, blank=True, validators=[RegexValidator(r'^\d{1,10}$')])
    ...
    weight = models.DecimalField(max_digits=5, decimal_places=2, default='', null=True, blank=True,verbose_name="Weight (kg)")
    height = models.DecimalField(max_digits=5, decimal_places=2, default='', null=True, blank=True,verbose_name="Height (cm)")
    ...
    water_quantity = models.DecimalField(max_digits=5, decimal_places=2, default=0, null=True, blank=True, verbose_name="Water Quantity (L)")
...
class BodyComposition(models.Model):
    ...
    # appointment = models.ForeignKey(Appointments, on_delete=models.CASCADE, related_name='body_compositions', blank=True, null=True)
...
class Appointments(models.Model):
    ...
    meal_plan = models.ForeignKey(MealPlan, on_delete=models.CASCADE, blank=True, null=True, related_name='appointments')
    date_assigned = models.DateTimeField()
    body_composition = models.ForeignKey(BodyComposition, on_delete=models.SET_NULL, blank=True, null=True, related_name='appointments')
    STATUS_CHOICES = (
        ('scheduled', 'Scheduled'),
        ('confirmed', 'Confirmed'),
        ('cancelled', 'Cancelled'),
    ...
    Appointment_description = models.TextField(default='', verbose_name="Appointment Description", blank=True, null=True)
